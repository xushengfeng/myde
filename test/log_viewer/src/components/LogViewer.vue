...
const _relatedUids = computed<Set<number>>(() => {
...
function _onLogClick(e: MouseEvent) {
    const target = e.target as HTMLElement;
    const el = target.closest<HTMLElement>("[data-uid]");
    if (el) {
        const uid = parseInt(el.dataset.uid ?? "-1", 10);
        if (uid >= 0) emit("obj-click", uid);
        return;
    }
    const lineEl = target.closest<HTMLElement>(".log-line");
    if (lineEl) {
        const idx = parseInt(lineEl.dataset.idx ?? "-1", 10);
        if (!Number.isNaN(idx)) emit("update:highlightLine", idx);
    }
}
</script>
