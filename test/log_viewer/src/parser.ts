...
export function parseLog(text: string): ParseResult {
    ...
    for (let i = 0; i < rawLines.length; i++) {
        ...
        if (m) {
            ...
            const objId = parseInt(objRef.substring(dotPos + 1), 10);
            ...
            while ((nm = nmRe.exec(args)) !== null) {
                ...
                const nInst = ensureInst(nType, parseInt(nm[2], 10), i, targetInst.uid, true);
                newIds.push({
                    type: nType,
                    id: parseInt(nm[2], 10),
                    uid: nInst.uid,
            ...
            const html = buildLineHtml({
                objType,
                objId,
                targetUid: targetInst.uid,
                method,
                args,
                newIds,
                argRefs,
...
function buildLineHtml(opts: {
    ...
    for (const n of newIds)
        tokens.push({ pos: n.start, end: n.end, uid: n.uid, cls: "kw-new", displayText: `new id ${n.type}#${n.id}` });
