...
type zLine = { ps: zLinePoint[]; width: number; color: string };
...
function generateZLinePath(zline: zLine): string {
    ...
    // 左侧轮廓：startLeft → 各中间点圆角 → endLeft
    let leftPath = "";
    if (startLeftFillet) {
        // 从切点 1（-n方向偏移）开始，画圆弧到切点 2（v方向偏移）
    ...
    for (let i = 1; i < len - 1; i++) {
        const d1 = edgeDirs[i - 1]; // 进入边方向
        const d2 = edgeDirs[i]; // 离开边方向
        const cross = vecCross(d1, d2);
    ...
    // 右侧轮廓：endRight → 各中间点圆角 → startRight
    let rightPath = "";
    if (endRightFillet) {
