...
tools.registerTool(
    ...
    ({ tipEl, showTip }) => {
        ...
        // 顶部显示电池状态
        const batteryIndicator = getIconXEl("battery", { size: 20, width: 24, height: 24 });
...
tools.registerTool(
    ...
    ({ tipEl, showTip }) => {
        const el = view("x")
            ...
            .add(getIconXEl("wifi", { size: 20, width: 24, height: 24 }));
...
tools.registerTool(
    ...
    ({ tipEl, showTip }) => {
        const el = view("x")
            .add(getIconXEl("blue", { size: 20, width: 24, height: 24 }).el)
            .style({ alignItems: "center" });
