...
export class dbusMenu {
    ...
    async init() {
        ...
        this.mainInterface.on<"a(ia{sv})a(ias)">("ItemsPropertiesUpdated", (_updatedProps, _removedProps) => {
            this.ev.emit("update");
        });

        // 监听整体菜单布局更新
        this.mainInterface.on<"ui">("LayoutUpdated", (_revision, _parent) => {
            this.ev.emit("update");
        });
    }
    private infc() {
    ...
    async getAllLayout() {
        ...
        async function buildMenuList(oneLayout: DBusType<"av">) {
            ...
            for (const _item of oneLayout) {
                ...
                const v = item[1];
                for (const [k, _v] of v) {
                    if (k === "type") itemObj.type = (_v.value as DBusTypes<"s">)[0] as "standard" | "separator";
                    else if (k === "label") itemObj.label = (_v.value as DBusTypes<"s">)[0];
                    else if (k === "enabled") itemObj.enabled = (_v.value as DBusTypes<"b">)[0];
                    else if (k === "visible") itemObj.visible = (_v.value as DBusTypes<"b">)[0];
                    else if (k === "icon-name") {
                        itemObj.iconUrl = async (op) => await getDesktopIcon((_v.value as DBusType<"s">)[0], op);
                    } else if (k === "icon-data") {
                        if (itemObj.iconUrl) continue;
                        const pngData = new Uint8Array((_v.value as DBusTypes<"ay">)[0]);
                        const blob = new Blob([pngData], { type: "image/png" });
                        const blobUrl = URL.createObjectURL(blob);
                        itemObj.iconUrl = () => Promise.resolve(blobUrl);
                    } else if (k === "shortcut")
                        itemObj.shortcut = (_v.value as DBusTypes<"a(as)">)[0][0]; // todo check
                    else if (k === "toggle-type")
                        itemObj.toggleType = (_v.value as DBusTypes<"s">)[0] as MenuItem["toggleType"];
                    else if (k === "toggle-state") itemObj.toggleState = (_v.value as DBusTypes<"i">)[0] === 1;
                    else if (k === "children-display" && (_v.value as DBusTypes<"s">)[0] === "submenu") {
