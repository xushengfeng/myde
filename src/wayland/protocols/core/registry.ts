 ...
 * 绑定时初始化改由模块声明 `globals[].onBind`，不再集中写 if 链。
 * 这样新增 global 只需在自己的模块里加一段，不必改中枢（原 P5）。
 */
export const registryModule = defineModule({
    ...
    requests: {
        "wl_registry.bind": (x, ctx) => {
            ...
            // 所以这里记到的始终是 undefined（与重构前一致，勿当成已生效的版本记录）
            ...
            // TODO Phase 5：xdg_shell 模块声明该 global 后，此分支随之删除
