...
`wl_surface:commit`的双缓冲与屏幕显示渲染的双缓冲有些不同，不是一个概念。

在commit之前还会有如设置attach、damage等，这一系列的设置组成了一个状态。试想：在客户端设置好一帧后继续设置下一帧，合成器不知道帧之间的区分，那damage之类的会错乱，所以需要commit标记。进一步，commit标记好收集的状态，不能再变动部分，即这个状态是不可分割的、原子的，前端的想法应该就是克隆到副本然后清空原本数据给下一帧添加各种数据，这里双缓冲是一个意思，不过底层实现不同而已。把pending的数据交给current，并给pending设置空数据。当然，如果在不同时间读取current的不同部分，那可能就不是一个状态了，这里原子性保证的是同一时间拿到的数据内部是不变的而已。双缓冲里面最明显的是buffer，buffer写入和读取时间都比其他数据长，current（或者attach后）buffer都不能变，不可能等到显示后再写入buffer，所以客户端一般有两个buffer。
